...
#' bgm2(pre_global_model_pa)


#' @export

bgm2<- function (tab_input,min_c,max_c,Er,CVr,CV_Cobs,CV_Iobs,n_proj=0){
  ...
  model_file<-paste0(path.package("demerstem"), "/model_BiomProd_WithScenario_JAGS2.txt")
...
data <- list("I_obs1" = tab_input$I_PA,"I_obs2" = tab_input$I_PI,"I_obs3" = tab_input$I_SC, "C_obs" = tab_input$C, "n_obs" = n_obs, "n_proj" = n_proj,
  ...
  monitor <- c( "B", "h", "D", "C",
                "r", "r_p", "K", "K_p", "q1","q2","q3", "sigma2p",
                "C_MSY", "C_MSY_p", "B_MSY", "h_MSY",
                "risk", "Over_C","Over_h",
                "C_e",
                "I_pred1","I_pred2","I_pred3", "C_pred")
