...
generalised_model <- function(table_Efox, graph_param, vect_ini=c(10, -0.5, 1), vect_lower=c(0.5, -1, 0.01), vect_upper=c(100, 10, 9), warning_control = FALSE, max_iteration = 50){
  ...
  plotGEN_U <- table_Efox %>% mutate(title = todo[length(list_graph) + 1,]) %>% ggplot() +
  ...
  plotGEN_U_2 <- table_Efox %>% mutate(title = todo[length(list_graph) + 1,]) %>% ggplot() +
  ...
  plotGEN_Y <- table_Efox %>% mutate(title = todo[length(list_graph) + 1,]) %>% ggplot() +
    ...
    theme_nice() + labs(x = "mE", y = "Catch") +
    geom_line(data = data, aes(x=mE_fox, y = Y_Efox), color="blue", size = 1.1) # D.a dit de prendre E

  list_graph[[length(list_graph) + 1]] <- plotGEN_Y

  plotGEN_Y_2 <- table_Efox %>% mutate(title = todo[length(list_graph) + 1,]) %>% ggplot() +
  ...
  plotGEN_pred <- table_Efox %>%  full_join(interval_confidence_pred[,c(11:13)], by  = 'Year') %>%
